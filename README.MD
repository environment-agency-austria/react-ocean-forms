...
***className*** (String, optional, default: undefined)
...
If set to true, all the fields will display only text instead of an input element. This is useful to re-use Fields in a check page.
...
## Field Component
Defines a form line containing a label and an input. Additionally it will render validation messages. If the user adds the required validator then it will mark the field as required as well.
...
### Lifecycle hooks
***getDisplayValue*** (value, { disabled, plaintext })
Called, when the field is loading it's value from the forms default values. Must return the value to display.

***getSubmitValue*** (value, { disabled, plaintext })
Called, when the field is submitting it's value to the form. Must return the value to submit.
...
***disableFocusOnSubmit*** (Boolean, optional, default: false)
If set to true the validation summary will stop automatically scrolling to itself when the user clicks on a submit button and the form is invalid.
