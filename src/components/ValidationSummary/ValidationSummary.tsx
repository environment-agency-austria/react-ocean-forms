...
export const ValidationSummary: React.FC<IValidationSummaryProps> = (props) => {
  ...
  const renderedErrors = errorList.map(([, value]) => renderFieldError(
    value.id,
    value.name,
    value.error,
    value.linkCallback,
  ...
  if (renderedErrors.length === 0) { return null; }

  const summary = (
    <div id={id} className="validation-summary">
      <h4 className="alert-heading" ref={headerRef}><FormText text={title} /></h4>
      <ul>
        {renderedErrors}
      </ul>
    </div>
